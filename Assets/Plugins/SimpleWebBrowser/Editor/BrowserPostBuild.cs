       ...
       // File.
        ...
        switch (target)
        {
            case BuildTarget.StandaloneWindows64:
            {
                ...
                string[] files=Directory.GetFiles(Application.dataPath+ @"\Plugins\SimpleWebBrowser\PluginServer\x64");
                foreach (var file in files)
                {
                    ...
                    files = Directory.GetFiles(Application.dataPath + @"\Plugins\SimpleWebBrowser\PluginServer\x64\locales");
                    foreach (var file in files)
                    {
                        if (!file.Contains("meta"))
                        {
            ...
            case BuildTarget.StandaloneWindows:
            {
                    ...
                    string[] files = Directory.GetFiles(Application.dataPath + @"\Plugins\SimpleWebBrowser\PluginServer\x86");
                    foreach (var file in files)
                    {
                        if (!file.Contains("meta"))
                        {
                    ...
                    Directory.CreateDirectory(Path.GetDirectoryName(path) + @"\PluginServer\locales");
                    files = Directory.GetFiles(Application.dataPath + @"\Plugins\SimpleWebBrowser\PluginServer\x86\locales");
                    foreach (var file in files)
                    {
                        if (!file.Contains("meta"))
                        {
